...
The integration uses `android-gles` in both repositories. Publish the companion branch before the engine branch so hosted CI can fetch the pinned game-source commit.
...
- Preserve exact GLES alpha-test comparisons, including cutout boundaries, and force required vertex buffers when an older configuration disables them.
- Repeat edge texels before block compression, preserve logical cube-map mip dimensions in cache headers, and clip compressed uploads to the addressed mip. Generated image caches use a new version so earlier malformed or darkened entries regenerate.
- Derive APK version, minimum API and SDL Java requirements from the staged native build; verify dependency architecture, API, 16 KiB alignment and license notices before packaging.
- Defer SigmaTouch label lookup until engine initialization and retain held actions until all overlapping control aliases are released. Optional hosts must supply the matching patched SDL callbacks.
- Remove obsolete extracted Android packages only after the current package completes, preserving saves and retrying interrupted extraction. Existing OpenAL intro/loop queues keep using their uploaded buffers without unnecessary PCM restoration.
...
The subsequent pre-publication review adds these checks:

- All **179 source checks** in the push profile pass, including four new portable regressions for GLES alpha state, mip uploads, image-block generation and the Android Activity's storage lifecycle. CI provisions JDK 17 for the Activity harness.
- Production GLES shaders pass 47 real ES 3.0 alpha/depth boundary outcomes; all 20 programs link. The production ETC2/EAC encoder and edge-padding helper pass 30 GPU upload/readback combinations. The cube producer passes logical-header, complete-block and ownership checks across 32 mip chains; padding checks cover 81 dimensions.
- The actual touch adapter passes overlapping-button and deferred-localization tests. The actual Activity passes initial extraction, reuse, interrupted-upgrade recovery and scoped cleanup checks. OpenAL payload tests preserve uploaded intro/loop playback when source files are unavailable while still restoring PCM for streaming.
- Windows and Android native targets rebuild and stage successfully; all 12 Windows native tests pass again. Fresh Windows SP OpenGL and MP Vulkan stock-map runs enter gameplay, produce inspected engine screenshots, pass their 12 renderer-tier cases and exit normally without new warning messages against the recorded baseline.
- The rebuilt Linux GLES client passes fresh stock SP and MP gameplay with both vertex/index buffer settings initially disabled. Initialization restores the required buffers, all 20 shaders link, GPU skinning remains available, and both runs pass 12 renderer-tier cases and exit without GL errors. Their engine screenshots were visually inspected; the temporary runtime mounts and keeper were removed afterward.
- APK configuration rejects mismatched native API, ABI, SDL Java version and invalid version codes. The APK takes version `0.13.1` from current staging. Its v2 signature, 16 KiB ZIP/native alignment, seven native libraries, both runtime packs, metadata, contributor credits and dependency notices are verified against staging. Custom-Activity lint passes with the supplied SDL sources excluded; the unfiltered dependency findings remain recorded below.

Review evidence is retained in `.tmp/android-gles-review-source-checks.log`, `.tmp/android-review-*.log`, `.tmp/review-gles-alpha-es30.log`, `.tmp/review-gles-texture-probe.log`, `.tmp/review-gles-linux-evidence/`, `.tmp/android-apk-verification.json`, and `.tmp/android-gles-review-windows-{sp-gl,mp-vulkan}/`.
...
- **SDL Android lint:** unfiltered SDL 3.4.10 lint reports 26 permission findings in optional Steam BLE paths, one API-29 pen-handler finding and one receiver-flag finding. Source and bytecode inspection confirms the version/permission guards; the standalone host does not enable Steam BLE without its required permission. These findings and the unfiltered report are retained locally rather than suppressed in upstream source. Physical-device qualification remains necessary.
