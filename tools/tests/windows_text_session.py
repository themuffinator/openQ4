...
def main():
    ...
    test=ROOT/'tools/tests/native/WindowsTextSessionTest.cpp'
    ...
    try:
        ...
        def case(name,mutant=False):
            ...
            command=[a.compiler]+[str(n) for n in common]+[str(obj)]+(['ole32.lib','uuid.lib'] if a.msvc_debug else ['-lole32','-luuid'])
