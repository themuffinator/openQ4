...
================
*/
void idAASBuild::ParseProcNodes( Lexer *src ) {
	int i;
...
bool idAASBuild::LoadProcBSP( const char *name, unsigned int minFileTime ) {
	...
	// the Quake 4 .proc header carries a quoted version and the map geometry CRC
	// between the file id and the first section; consume both before scanning
	if ( !src->ReadToken( &token ) ) {
		common->Warning( "idAASBuild::LoadProcBSP: %s is missing version", fileName.c_str() );
		delete src;
		return false;
	}

	if ( !src->ExpectTokenType( TT_NUMBER, TT_INTEGER, &token ) ) {
		common->Warning( "idAASBuild::LoadProcBSP: %s has no map file CRC", fileName.c_str() );
		delete src;
		return false;
	}
...
/*
============
idAASBuild::WriteDummyFile

Writes an AAS file that carries the map CRC and the AAS type settings but no
navigation data.  idAASFile::IsDummyFile() recognises it, so the game treats
the type as "not used by this map" instead of reporting missing navigation.
============
*/
bool idAASBuild::WriteDummyFile( const idStr &fileName, unsigned int mapFileCRC ) {
	idStr name;
	idAASFileLocal dummy;

	dummy.settings = *aasSettings;

	name = fileName;
	name.SetFileExtension( aasSettings->fileExtension );

	return dummy.Write( name, mapFileCRC );
}
...
bool idAASBuild::Build( const idStr &fileName, const idAASSettings *settings ) {
	...
	// Quake 4 maps keep func_group entities in the editable source; resolve them
	// into world geometry exactly like dmap and the runtime map loader do, so the
	// navigation mesh is built from the same brushes as the .proc/.cm.
	mapFile->Resolve();

	// check if this map has any entities that use this AAS file
	if ( !CheckForEntities( mapFile, entityClassNames ) ) {
		// Quake 4 writes a placeholder file for every declared AAS type even when
		// a map has nothing that uses it, so the runtime can tell "this map has no
		// aasXX navigation by design" (idAASFile::IsDummyFile, which loads quietly)
		// from "the aasXX file is missing or stale", which warns on every load.
		WriteDummyFile( fileName, mapFile->GetGeometryCRC() );
		delete mapFile;
	...
	if ( !bsp.RemoveOutside( mapFile, AREACONTENTS_SOLID, entityClassNames ) ) {
		bsp.LeakFile( name );
		delete mapFile;
		common->Printf( "%s has no outside\n", name.c_str() );
		return false;
	}
...
Shared option scanner for runAAS/runReach.  Unknown arguments (including the
dmap switches when dmap forwards its own command line) are ignored, and the
last argument is taken as the map name.
============
*/
static int ParseOptions( const idCmdArgs &args, idAASSettings &settings ) {
	int i;
	idStr str;
...
The set of AAS types a map is compiled for comes from the shipped 'aas_types'
entityDef ("type1".."typeN").  Resolve it through declManager rather than
gameEdit so dmap can compile navigation without a loaded game module, and so a
dedicated-server build behaves the same as the client.
============
*/
static const idDict *AAS_GetTypesDict( void ) {
	const idDeclEntityDef *decl = static_cast<const idDeclEntityDef *>(
			declManager->FindType( DECL_ENTITYDEF, "aas_types", false ) );
	if ( !decl ) {
		return NULL;
	}
	return &decl->dict;
...
============
*/
static const idDict *AAS_GetSettingsDict( const char *typeName ) {
	const idDeclEntityDef *decl = static_cast<const idDeclEntityDef *>(
			declManager->FindType( DECL_ENTITYDEF, typeName, false ) );
	if ( !decl ) {
		return NULL;
	}
	return &decl->dict;
...
/*
============
AAS_NormalizeMapName

Accepts what the user typed (with or without a "maps/" prefix, backslashes, or a
file extension) and returns the extension-less, forward-slashed relative path
the AAS builder expects.
============
*/
static idStr AAS_NormalizeMapName( const char *name ) {
	idStr mapName = name;
...
Compiles every AAS type declared in 'aas_types' for one map.  This is the shared
core behind the runAAS command and dmap's automatic navigation pass, so it
deliberately leaves the caller's warning list and refresh state alone.

'mapName' is a relative, extension-less map path such as "maps/game/hangar2".
'optionArgs' may be NULL; when supplied, its switches are applied on top of each
AAS type's shipped settings.
============
*/
bool RunAASForMap( const idStr &mapName, const idCmdArgs *optionArgs ) {
	...
	const idDict *dict = AAS_GetTypesDict();
	if ( !dict ) {
		common->Warning( "Unable to find entityDef for 'aas_types'; no AAS navigation was compiled" );
		return false;
	}

	for ( const idKeyValue *kv = dict->MatchPrefix( "type" ); kv != NULL; kv = dict->MatchPrefix( "type", kv ) ) {
		const idDict *settingsDict = AAS_GetSettingsDict( kv->GetValue() );
		...
		if ( optionArgs ) {
			ParseOptions( *optionArgs, settings );
		}

		if ( !aas.Build( mapName, &settings ) ) {
			failed = true;
		}
		numCompiled++;
	}
	...
	if ( !numCompiled ) {
		common->Warning( "'aas_types' declares no AAS types; no AAS navigation was compiled" );
		return false;
	}
...
void RunAAS_f( const idCmdArgs &args ) {
	if ( args.Argc() <= 1 ) {
		common->Printf( "runAAS [options] <mapfile>\n"
					...
					"  -noOptimize        = do not optimize the resulting AAS file.\n" );
		return;
	}
	...
	RunAASForMap( AAS_NormalizeMapName( args.Argv( args.Argc() - 1 ) ), &args );
...
void RunAASDir_f( const idCmdArgs &args ) {
	...
	for ( i = 0; i < mapFiles->GetNumFiles(); i++ ) {
		...
		RunAASForMap( AAS_NormalizeMapName( idStr( "maps/" ) + args.Argv( 1 ) + "/" + mapFiles->GetFile( i ) ), NULL );
	}
...
void RunReach_f( const idCmdArgs &args ) {
	...
	const idDict *dict = AAS_GetTypesDict();
	if ( !dict ) {
		common->Warning( "Unable to find entityDef for 'aas_types'" );
	} else {
		mapName = AAS_NormalizeMapName( args.Argv( args.Argc() - 1 ) );
		...
		for ( const idKeyValue *kv = dict->MatchPrefix( "type" ); kv != NULL; kv = dict->MatchPrefix( "type", kv ) ) {
			...
			settings = idAASSettings();
			settings.FromDict( kv->GetValue(), settingsDict );
			ParseOptions( args, settings );
			aas.BuildReachability( mapName, &settings );
