...
bool idAASBuild::GetAreaForLeafNode( idBrushBSPNode *node, int *areaNum ) {
	...
	// Quake 4's aasArea_t carries fields this pass does not fill in (bounds,
	// center and ceiling are derived on load; the tactical feature index and the
	// runtime obstacle marker are not compiled), so start from a cleared area
	// rather than leaving them holding stack garbage.
...
================
*/
// sizeEstimate_t is declared with the rest of the AAS file types in AASFile.h
