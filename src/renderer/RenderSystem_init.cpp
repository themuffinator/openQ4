...
idCVar r_shadowMapImageSize( "r_shadowMapImageSize", "512", CVAR_RENDERER | CVAR_INTEGER | CVAR_ARCHIVE, "base shadow map tile resolution", 64, 4096, idCmdSystem::ArgCompletion_Integer<64,4096> );
...
idCVar r_shadowMapPolygonFactor( "r_shadowMapPolygonFactor", "2", CVAR_RENDERER | CVAR_FLOAT | CVAR_ARCHIVE, "polygon offset factor for shadow-map depth rendering" );
idCVar r_shadowMapPolygonOffset( "r_shadowMapPolygonOffset", "0.1", CVAR_RENDERER | CVAR_FLOAT | CVAR_ARCHIVE, "polygon offset units for shadow-map depth rendering" );
...
idCVar r_shadowMapSplitWeight( "r_shadowMapSplitWeight", "0.9", CVAR_RENDERER | CVAR_FLOAT | CVAR_ARCHIVE, "blend between logarithmic and linear split spacing for parallel-light cascades" );
...
idCVar r_shadowMapOccluderSource( "r_shadowMapOccluderSource", "0", CVAR_RENDERER | CVAR_INTEGER | CVAR_ARCHIVE, "occluder source policy for mapped shadows (0 auto/interactions-first, 1 shadow-surfs-first, 2 interactions-only)", 0, 2, idCmdSystem::ArgCompletion_Integer<0,2> );
idCVar r_shadowMapDebugFlow( "r_shadowMapDebugFlow", "0", CVAR_RENDERER | CVAR_INTEGER, "shadow-map debug flow verbosity (0 off, 1 summary, 2 per-light pipeline logs, 3 = include invalid occluder tri details)", 0, 3, idCmdSystem::ArgCompletion_Integer<0,3> );
idCVar r_shadowMapDebugLight( "r_shadowMapDebugLight", "-1", CVAR_RENDERER | CVAR_INTEGER, "shadow-map debug light filter (-1 = all, otherwise render-light index/lightId)", -1, 65535, idCmdSystem::ArgCompletion_Integer<-1,65535> );
idCVar r_shadowMapDebugLogInterval( "r_shadowMapDebugLogInterval", "30", CVAR_RENDERER | CVAR_INTEGER, "frame interval for per-light shadow-map debug logs when r_shadowMapDebugFlow=2 (1 = every frame)", 1, 600, idCmdSystem::ArgCompletion_Integer<1,600> );
idCVar r_shadowMapStrictMappedPath( "r_shadowMapStrictMappedPath", "0", CVAR_RENDERER | CVAR_BOOL, "1 = keep mapped interaction path even when map render fails (debug), 0 = fallback to stencil" );
